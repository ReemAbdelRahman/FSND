...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql+psycopg2://postgres:password@localhost:5432/project1' 
#----------------------------------------------------------------------------#
# Models.
#----------------------------------------------------------------------------#
...
class Venue(db.Model):
    ...
    shows = relationship("Show",backref="venues")   
...
class Show(db.Model):
  ...
  artists = db.relationship('Artist', secondary = show_table)
...
def venues():
  ...
  city_dict = {}
  for v in venues:
    if not v.city in city_dict.values():
      city_dict['city'] = v.city
      city_dict['state'] = v.state
      city_dict['venues'].append({"id": v.id, "name": v.name, "num_upcoming_shows":v.shows})
    else:
      city_dict['venues'].append({"id": v.id, "name": v.name, "num_upcoming_shows":v.shows})
...
def show_venue(venue_id):
  ...
  venue = Venue.query.filter_by(id = )
  jsonify(body)
  data1={
    "id": 1,
    "name": "The Musical Hop",
...
@app.route('/artists')
def artists():
  # TODO12: replace with real data returned from querying the database
...
def search_artists():
  ...
  # TODO13: implement search on artists with partial string search. Ensure it is case-insensitive.
...
def show_artist(artist_id):
  # shows the venue page with the given venue_id
  # TODO14: replace with real venue data from the venues table, using venue_id
  data1={
    "id": 4,
    "name": "Guns N Petals",
    "genres": ["Rock n Roll"],
...
def edit_artist(artist_id):
  form = ArtistForm()
  # TODO15: populate form with fields from artist with ID <artist_id>

  return render_template('forms/edit_artist.html', form=form, artist=artist)

@app.route('/artists/<int:artist_id>/edit', methods=['POST'])
def edit_artist_submission(artist_id):
  # TODO16: take values from the form submitted, and update existing
...
def create_artist_submission():
  ...
  # TODO19: insert form data as a new Venue record in the db, instead
  # TODO20: modify data to be the data object returned from db insertion
...
def create_show_submission():
  ...
  # TODO24: on unsuccessful db insert, flash an error instead.
  error = True
